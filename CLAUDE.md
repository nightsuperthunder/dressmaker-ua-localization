...
- Player (Dressmaker) is **female** (decided): «Кравчиня», vocative «Кравчине», feminine forms in her lines and when addressed. Gossip-column narrator («one») is also female. Use feminitives for women (художниця, продавчиня, мисткиня). «Cloth Vendor» (insult) = «продавчиня ганчір’я».
...
| Gender check | `python scripts/context_review.py --gender --model gemma4:26b-a4b-it-q4_K_M --no-think` | Gender/feminitive-only review → work/gender_review.csv. Fixes >6 changed words or failing validate() are kept as «ПЕРЕВІР ВРУЧНУ (пропозиція моделі: …)». Full dialogue run ~15 min, 46 hits, ~30 genuine (speaker gender often needs manual context check; model also 'fixes' male speakers). Better: apply genuine ones as manual word replacements rather than importing the CSV. |
...
tags multiset equal, placeholders multiset equal, prefix kept, `\n` count, `\:` rule, Cyrillic present, ≥3 leftover English words, length sanity, placeholder-only / roman-numeral strings must stay identical, no spurious outer «…» (also with trailing punctuation/space, e.g. `«Так». `), no trailing space the source lacks, balanced «».
...
- Stats: ok 4872, manual 679, skip 335, error 0 (2 known validator false positives: `Discord`, credits line with Latin names). v1.2.4 unwrapped 327 dialogue lines the model had wrapped in «…» (the validator missed them because of a trailing space / punctuation after »), fixed broken nested quotes in fabric names (style: «Оксамит деворе «Медуза»», «Шовк із принтом «Беладона»», «Жатий оксамит «Міраж»»).
...
- Releases: v1.0.0 … **v1.2.7** (latest). Listed on kuli.com.ua? — form data: Steam appid **4019220**, platform Windows only, category «Неофіційні», type «Текст», author `nightsuperthunder`, screenshots in `docs/screenshots/*.png` (raw.githubusercontent links).
- Mod verified in game (main menu, settings, dialogue, sketchbook screenshots in docs/screenshots).
- TODO / open: better Cyrillic fonts (EB Garamond has official Cyrillic under OFL — drop .ttf into `mod/fonts/`, filename = latin font name or `default.ttf`; needs a download); localized images (5 in `Images` asset table) stay English; ~120 `{0}` colour/fabric sentences need in-game checking; grammar/gender errors are invisible to the issue finder — only players catch them.
...
- Text that doesn't fit the game's fixed-width buttons (Ukrainian words are longer): `ApplyAutoSize` turns on TMP auto-sizing for our locale only — authored `fontSize` becomes `fontSizeMax`, min = `AutoSizeMinRatio` (config, default 0.6), floored at 6 but never above the authored size (world-space texts — sewing accuracy dial, mannequin measurements — have fontSize < 6; «min 6» blew them up into a huge white blob, fixed in 1.2.4). Linked-overflow texts (newspaper `GossipReport` columns, `linkedTextComponent`) are excluded — auto-size shrank the first column to fit the whole article. Applied on `sceneLoaded` (`Resources.FindObjectsOfTypeAll<TMP_Text>`) and via a Harmony postfix on `TextMeshProUGUI.OnEnable` for runtime-created labels. Verified in game. Fallback for bad cases: shorten the label in translations (`manual`).
